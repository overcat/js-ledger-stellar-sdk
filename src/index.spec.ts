...
test("getAppConfiguration (hash signing not enabled)", async () => {
  const transport = await openTransportReplayer(
    RecordStore.fromString(`
    => e006000000
    <= 000402039000
...
test("getPublicKey (accountId = 0, not display)", async () => {
  const transport = await openTransportReplayer(
    RecordStore.fromString(`
...
test("getPublicKey (accountId = 0, display)", async () => {
  const transport = await openTransportReplayer(
    RecordStore.fromString(`
...
test("getPublicKey (accountId = 4096, not display)", async () => {
  const transport = await openTransportReplayer(
    RecordStore.fromString(`
...
test("getPublicKey (accountId = 4096, display)", async () => {
  const transport = await openTransportReplayer(
    RecordStore.fromString(`
...
test("signHash (buffer hash, accountId = 0)", async () => {
...
test("signHash (string hash without 0x, accountId = 0)", async () => {
  const hash = "4b480b455a7ee154c33651819e3ce2ceb6bcd9dda78887777c4d2718c5cd04cd";
  const expectSignature =
    "WLCaR4HoduZIMBZCOaZ4QJYUIknJXe2/28CXPAC2f2NuKXMeOG5Q05fQ3xW91uI3Qk8jE33HvLyODaYK1MCoDQ==";
  const transport = await openTransportReplayer(
...
test("signHash (string hash with 0x, accountId = 0)", async () => {
  const hash = "4b480b455a7ee154c33651819e3ce2ceb6bcd9dda78887777c4d2718c5cd04cd";
  const expectSignature =
    "WLCaR4HoduZIMBZCOaZ4QJYUIknJXe2/28CXPAC2f2NuKXMeOG5Q05fQ3xW91uI3Qk8jE33HvLyODaYK1MCoDQ==";
  const transport = await openTransportReplayer(
...
test("signHash (buffer hash, accountId = 4096)", async () => {
  const hash = "4b480b455a7ee154c33651819e3ce2ceb6bcd9dda78887777c4d2718c5cd04cd";
  const expectSignature =
    "rItjXm9p2cpGd4O3NGxp5i95/+nUUfwl4fI9snN3xHot5BNdmLJ0BTa84mrMomTf8dzildS7/GMapydStTevBQ==";
  const transport = await openTransportReplayer(
    RecordStore.fromString(`
...
test("signTransaction (only one chunk, accountId = 0)", async () => {
...
test("signTransaction (only one chunk, accountId = 4096)", async () => {
...
test("signTransaction (multiple chunks, accountId = 0)", async () => {
  const kp = Keypair.fromSecret("SAIYWGGWU2WMXYDSK33UBQBMBDKU4TTJVY3ZIFF24H2KQDR7RQW5KAEK");
  const account = new Account(kp.publicKey(), "103720918407102567");
  const transaction = new TransactionBuilder(account, {
    fee: BASE_FEE,
